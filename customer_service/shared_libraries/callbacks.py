...
def after_session_callback(invocation_context: InvocationContext) -> None:
    """Callback that runs after each session ends to sync to CRM.
    
    This callback ensures that any relevant session data is properly 
    synced to the CRM system after the conversation concludes.
    ...
    try:
        ...
        if "customer_profile" in invocation_context.state:
            customer_data = invocation_context.state.get("customer_profile", {})
            customer_id = customer_data.get("customer_id", None)
            
            if customer_id:
                # Extract session notes and summary
                conversation_history = invocation_context.state.get("conversation_history", [])
                interaction_summary = "Customer interaction via AI agent"
                ...
                if conversation_history and len(conversation_history) > 0:
                    ...
                    for entry in conversation_history:
                        if isinstance(entry, dict) and "message" in entry:
                            ...
                            if "rental" in message.lower():
                                topics.add("equipment rental")
                            if "quote" in message.lower():
                                topics.add("quote request")
                            if "avail" in message.lower():
                                topics.add("availability check")
                            if "schedule" in message.lower():
                                topics.add("scheduling")
                                
                    if topics:
                        interaction_summary = f"Customer discussed: {', '.join(topics)}"
                
                # Update the CRM with conversation data
                update_result = crm_sync.update_customer_status(
                ...
                if update_result:
                    logger.info(f"CRM updated successfully for customer: {customer_id}")
                else:
                    logger.error(f"Failed to update CRM for customer: {customer_id}")
    except Exception as e:
        logger.error(f"Error in after_session_callback: {e}")
